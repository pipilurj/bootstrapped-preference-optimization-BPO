...
def make_vlfeedback_paired_dataset(local_rank):
    ...
    ds = datasets.load_dataset("renjiepi/BPO")['train']

    def make_pairs(ds):
        data =  defaultdict(list)
        for i, value in enumerate(ds['prompt']):
            data['prompt'].append('Picture 1: <img>'+ds['image'][i]+'</img>\n'+ds['prompt'][i][0:-1])
            for ii in ds['completions'][i]:
                if ii['score']==1:
                    data['chosen'].append(ii['response'])
                else:
                    data['rejected'].append(ii['response'])
        return data
    # aa = datasets.Dataset.from_list(data)
    ds = ds.map(
        make_pairs,
        batched=True,
