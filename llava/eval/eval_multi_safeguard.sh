...
if [ "$#" -ne 6 ]; then
  echo "Usage: $0 <model_path> <answers_root> <num_chunks> <temperature> <harm_detector> <detoxifier>"
  exit 1
fi
...
scenes=("04-Physical_Harm" "05-EconomicHarm" "06-Fraud" "07-Sex" "08-Political_Lobbying" "09-Privacy_Violence" "10-Legal_Opinion" "11-Financial_Advice" "12-Health_Consultation" "13-Gov_Decision")
#scenes=("07-Sex" "08-Political_Lobbying" "09-Privacy_Violence" "10-Legal_Opinion" "11-Financial_Advice" "12-Health_Consultation" "13-Gov_Decision")
...
for scene in "${scenes[@]}"; do
  ...
  for ((chunk_id = 0; chunk_id < N; chunk_id++)); do
    ...
    CUDA_VISIBLE_DEVICES="$chunk_id" python llava/eval/robustness_eval.py --model-path "$model_path" --scene "$scene" --answers_file "$answer_path" --num-chunks "$N" --chunk-idx "$chunk_id" --temperature "$temperature" --harm_detector "$harm_detector" --detoxifier "$detoxifier"&
