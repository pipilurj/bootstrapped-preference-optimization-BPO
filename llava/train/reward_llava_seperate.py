...
class LLaVADPODataCollator(DPODataCollatorWithPadding):
    ...
    def tokenize_batch_element(
        ...
        """
        batch = {}
        response_0_conv = make_conv(prompt, response_0)
        response_1_conv = make_conv(prompt, response_1)
        # processing image
        if "img" in prompt:
            ...
            response_0_conv,  response_1_conv = preprocess_multimodal([copy.deepcopy(response_0_conv), copy.deepcopy(response_1_conv)], data_args=self.data_args)
        # tokenize
        response_0_conv, prompt_dict = preprocess(
            response_0_conv,
            self.tokenizer,
            has_image=True)
        response_1_conv, _ = preprocess(
            response_1_conv,
            self.tokenizer,
            has_image=True)

        if chosen_help == 0:
            chosen_help_conv = response_0_conv
            rejected_help_conv = response_1_conv
        else:
            chosen_help_conv = response_1_conv
            rejected_help_conv = response_0_conv
        if chosen_faith == 0:
            chosen_faith_conv = response_0_conv
            rejected_faith_conv = response_1_conv
        else:
            chosen_faith_conv = response_1_conv
            rejected_faith_conv = response_0_conv
        for k, toks in {
            "chosen_help": chosen_help_conv,
            "rejected_help": rejected_help_conv,
            "chosen_faith": chosen_faith_conv,
            "rejected_faith": rejected_faith_conv,
            "prompt": prompt_dict,
        }.items():
    ...
    def __call__(self, features):
        ...
        for feature in features:
            ...
            response_0 = feature["response_0"]
            response_1 = feature["response_1"]
            chosen_help = feature["chosen_help"]
            chosen_faith = feature["chosen_faith"]

            batch_element = self.tokenize_batch_element(prompt, response_0, response_1, chosen_help, chosen_faith)
    ...
    def make_batch_pairs(sample):
        ...
        for sample_idx, comps in enumerate(sample["completions"]):
            ...
            for comp_idx1, comp_idx2 in combinations(range(len(comps["annotations"])), 2):
                ...
                # get average scores
                try:
                    avg_score1_help = float(anno1["Helpfulness"]["Rating"])
                    avg_score2_help = float(anno2["Helpfulness"]["Rating"])
                    avg_score1_faith = float(anno1["Visual Faithfulness"]["Rating"])
                    avg_score2_faith = float(anno2["Visual Faithfulness"]["Rating"])
                except ValueError:
                    continue

                # get chosen and rejected responses
                if avg_score1_help > avg_score2_help:
                    chosen_help = 0
                elif avg_score2_help > avg_score1_help:
                    chosen_help = 1
                else:
                    continue

                # get chosen and rejected responses
                if avg_score1_faith > avg_score2_faith:
                    chosen_faith = 0
                elif avg_score2_faith > avg_score1_faith:
                    chosen_faith = 1
                else:
                    continue
                converted_sample["prompt"].append(prompt)
                converted_sample["response_0"].append(comps["response"][comp_idx1])
                converted_sample["response_1"].append(comps["response"][comp_idx2])
                converted_sample["chosen_help"].append(chosen_help)
                converted_sample["chosen_faith"].append(chosen_faith)
...
class CustomRewardTrainer(RewardTrainer):
    ...
    def concatenated_inputs(self, batch) -> Dict[str, torch.LongTensor]:
        concatenated_batch_help, concatenated_batch_faith = {}, {}
        max_length = max(batch["chosen_help_input_ids"].shape[1], batch["rejected_help_input_ids"].shape[1])

        for k in batch:
            if k.startswith("chosen_help") and isinstance(batch[k], torch.Tensor):
                pad_value = self.tokenizer.pad_token_id
                concatenated_key = k.replace("chosen", "concatenated")
                concatenated_batch_help[concatenated_key] = pad_to_length(batch[k], max_length, pad_value=pad_value)
        for k in batch:
            if k.startswith("rejected_help") and isinstance(batch[k], torch.Tensor):
                pad_value = self.tokenizer.pad_token_id
                concatenated_key = k.replace("rejected", "concatenated")
                concatenated_batch_help[concatenated_key] = torch.cat(
                    (
                        concatenated_batch_help[concatenated_key],
        ...
        for k in batch:
            if k.startswith("chosen_faith") and isinstance(batch[k], torch.Tensor):
                pad_value = self.tokenizer.pad_token_id
                concatenated_key = k.replace("chosen", "concatenated")
                concatenated_batch_faith[concatenated_key] = pad_to_length(batch[k], max_length, pad_value=pad_value)
        for k in batch:
            if k.startswith("rejected_faith") and isinstance(batch[k], torch.Tensor):
                pad_value = self.tokenizer.pad_token_id
                concatenated_key = k.replace("rejected", "concatenated")
                concatenated_batch_faith[concatenated_key] = torch.cat(
                    (
                        concatenated_batch_faith[concatenated_key],
                        pad_to_length(batch[k], max_length, pad_value=pad_value),
                    ),
                    dim=0,
                ).to(self.accelerator.device)
        return concatenated_batch_help, concatenated_batch_faith

    def concatenated_forward(
        self, model, batch):
        concatenated_batch_help, concatenated_batch_faith = self.concatenated_inputs(batch)
        len_chosen = batch["chosen_labels"].shape[0]
        images = batch["images"].repeat(4, 1,1,1)
        outputs = model(
            concatenated_batch["concatenated_input_ids"],
            attention_mask=concatenated_batch["concatenated_attention_mask"],
            images=images,
        )
        reward_help, reward_faith = outputs[0], outputs[1]
        rewards_chosen_help, rewards_rejected_help, rewards_chosen_faith, rewards_rejected_faith = rewards[:len_chosen], rewards[len_chosen:len_chosen*2], rewards[len_chosen*2:len_chosen*3], rewards[len_chosen*3:]
        return rewards_chosen_help, rewards_rejected_help, rewards_chosen_faith, rewards_rejected_faith
    ...
    def compute_loss(
        ...
        rewards_chosen_help, rewards_rejected_help, rewards_chosen_faith, rewards_rejected_faith = self.concatenated_forward(
            model, inputs
        )
        # calculate loss, optionally modulate with margin
        loss_help = -nn.functional.logsigmoid(rewards_chosen_help, rewards_rejected_help).mean()
        loss_faith = -nn.functional.logsigmoid(rewards_chosen_faith, rewards_rejected_faith).mean()
        loss = loss_help + loss_faith
        if return_outputs:
...
def train():
    ...
    if model_args.vision_tower is not None:
        model = LlavaLlamaForSequenceClassificationSep.from_pretrained(
