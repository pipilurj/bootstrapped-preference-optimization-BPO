...
if [ "$#" -ne 7 ]; then
    echo "Usage: $0  <model_path> <question_path> <base_answer_path> <image_folder> <N> <temperature> <start_gpu>"
    exit 1
fi
...
do
    ...
    CUDA_VISIBLE_DEVICES="$gpu" python llava/eval/model_vqa_loader.py --model-path "$model_path" --question-file "$question_path" --answers-file "$answer_path" --num-chunks "$N" --chunk-idx "$chunk_id" --image-folder "$image_folder" --temperature "$temperature" &
...
merged_file="${base_answer_path}.jsonl"
...
for ((i=0; i<N; i++)); do
  ...
  cat "$input_file" >> "${base_answer_path}.jsonl"
done
...
python llava/eval/eval_pope.py \
    ...
    --result-file "${base_answer_path}.jsonl"
