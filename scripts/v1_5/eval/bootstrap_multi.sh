...
do
    ...
    CUDA_VISIBLE_DEVICES="$gpu" python llava/eval/bootstrap.py --model-base "$model_base" --model-path "$model_path" --question-file "$question_path" --answers-file "$answer_path" --num-chunks "$N" --chunk-idx "$chunk_id" --image-folder "$image_folder" --num-samples "$num_samples" --subset "$subset" --temperature "$temperature" &
