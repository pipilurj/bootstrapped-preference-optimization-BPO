...
if [ "$#" -ne 8 ]; then
    echo "Usage: $0 <model_path> <question_path> <base_answer_path> <image_folder> <N> <temperature> <start_gpu> <TYPE>"
    exit 1
fi
...
do
    ...
    CUDA_VISIBLE_DEVICES="$gpu" python llava/eval/model_vqa_mathbench.py --model-path "$model_path" --question-file "$question_path" --answers-file "$answer_path" --num-chunks "$N" --chunk-idx "$chunk_id" --image-folder "$image_folder" --temperature "$temperature"  --question_type "$TYPE" &
